...
export default function CommandCenter({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    return [
      ...
      { id: "corona", title: "Open CORONA Lab", meta: "Explore · Heesch lab — draw a shape, ring it, climb to five", keywords: "corona heesch tiling tiler polyomino pentomino hex corona ring witness frontier contest layr lab", href: "/corona" },
      ...
      { id: "db", title: "Open DB Ladder", meta: "Data · 50 problems, SELECT to transactions", keywords: "db sql database ladder query join index window function cte transaction upsert sqlite", href: "/db" },
      ...
      { id: "foundry", title: "Enter the Foundry", meta: "Studio · AI production workshop", keywords: "foundry ai production engineering workshop station engine tokenizer tokens sampling attention serving retrieval rag agent guardrails llm systems operate", href: "/foundry" },
      { id: "foundry-tokenizer", title: "Foundry · Token Bench", meta: "Forge · BPE tokenizer, token budgets, pricing", keywords: "bpe byte pair encoding tokenizer merges vocabulary subword token budget context window pricing invoice", href: "/foundry/tokenizer" },
      { id: "foundry-sampling", title: "Foundry · Sampling Deck", meta: "Forge · temperature, top-k, nucleus sampling", keywords: "sampling decoding temperature top-k top-p nucleus softmax entropy degeneration random llm dials", href: "/foundry/sampling" },
      { id: "foundry-attention", title: "Foundry · Attention Lens", meta: "Forge · Q·K·V, softmax, positional encoding", keywords: "attention self-attention head softmax positional encoding lost in the middle context placement transformer", href: "/foundry/attention" },
      { id: "foundry-serving", title: "Foundry · Serving Floor", meta: "Forge · KV cache, batching, quantization", keywords: "serving inference server kv cache quantization fp16 q8 q4 continuous batching pagedattention vllm ttft latency throughput sla", href: "/foundry/serving" },
      { id: "foundry-rag", title: "Foundry · Retrieval Bench", meta: "Forge · BM25, chunking, context budget", keywords: "rag retrieval bm25 embedding cosine similarity chunking hybrid search grounded context budget keyword stuffing", href: "/foundry/rag" },
      { id: "foundry-agents", title: "Foundry · Agent Loop", meta: "Forge · ReAct loop, guardrails, injection", keywords: "agent react tool calls loop guardrails prompt injection context engineering trace retries owasp security", href: "/foundry/agents" },
