...
export function UnderstandStage({ lesson, saved, onComplete, onProbePass }: Props) {
  ...
  return (
    <StageShell stage="understand" title={lesson.title} move={lesson.stageMoves.understand}>
      ...
      <div className="example-strip" aria-label="Worked examples">
        {s.examples.map(ex => (
          <div key={ex.id} className="example-chip">
            <code>{ex.given}</code>
            <span>→</span>
            <code className="example-result">{ex.result}</code>
          </div>
        ))}
      </div>
      ...
      <div className="prediction-card">
        ...
        <div className="probe-options">
          {s.prediction.options!.map(o => (
            <OptionRow
              ...
              onClick={() => { if (!locked) setPicked(o.value) }}
            >
              {o.label}
            </OptionRow>
          ))}
        ...
        {!locked ? (
          <GhostButton disabled={!picked} onClick={() => setLocked(true)}>
