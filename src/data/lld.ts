...
export const STAGES_LLD = [
  ...
  { id: 10, name: "Concurrency", desc: "shared state under interleavings" },
]
...
export const PROBLEMS_LLD: Problem[] = [
  ...
  // A deterministic round-robin machine replaces real threads: Pyodide is
  // single-threaded, and deterministic interleavings teach the reasoning
  // better than a nondeterministic scheduler ever could.
  {
    id: 51, stage: 10, title: "The Lost Update", pattern: "race-condition", skill: "read-modify-write interleaves badly",
    statement: "Build the core of a deterministic concurrency simulator. Machine has value=0 and per-thread registers. run(threads) goes round-robin: each pass, every thread attempts its next op — step(tid, op) must return True if the op executed, False if the thread stays blocked. Implement two ops in step: ('read',) copies value into the thread's register; ('add', n) sets value = register + n. No blocking exists yet — always execute.",
    examples: [
      { input: "A: [read, add 1], B: [read, add 1], round-robin", output: "final value 1 — not 2", explain: "both read 0, both write 1: B's write erases A's" },
    ],
    why: "This is the race condition, frozen in amber. Real threads interleave at arbitrary points; the simulator interleaves at op boundaries — same disease, deterministic symptoms. Round-robin: A reads 0 into its register, B reads 0 into ITS register, A adds (value=1), B adds (value = B's register 0 + 1 = 1). A's increment evaporated — the lost update. Every concurrency bug you'll ever debug is some shape of this: read and write separated by someone else's write.",
    starterCode: "class DeadlockError(Exception):\n    pass\n\n# A tiny deterministic concurrency simulator: \"threads\" are op-lists,\n# run() goes round-robin, step() returns True if the op executed.\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        # ('read',) copies value into this thread's register\n        # ('add', n) sets value = register + n\n        pass",
    hints: [
      "read: self.registers[tid] = self.value, return True.",
      "add: self.value = self.registers[tid] + op[1], return True.",
      "Trace the race schedule by hand: both registers hold 0 when both adds run.",
    ],
    solution: "class DeadlockError(Exception):\n    pass\n\n# A tiny deterministic concurrency simulator: \"threads\" are op-lists,\n# run() goes round-robin, step() returns True if the op executed.\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"add\":\n            self.value = self.registers[tid] + op[1]\n            return True\n        return False",
    walkthrough: "The machinery you just built is the whole stage's instrument: round-robin scheduling is a worst-case-but-fair interleaving, and registers model the fact that every thread computes on its own view (CPU register / local variable) before writing back. Trace it once slowly: pass 1 — A reads 0, B reads 0. Pass 2 — A writes 0+1=1, B writes 0+1=1. Two increments entered, one survived. The single-threaded second assertion shows the same ops serialized give 2 — the delta IS the bug. Next problem: make read-modify-write indivisible.",
    testCode: "m = Machine()\nassert m.run({'A': [('read',), ('add', 1)], 'B': [('read',), ('add', 1)]}) == 1, 'interleaved adds must lose one'\nm2 = Machine()\nassert m2.run({'A': [('read',), ('add', 1), ('read',), ('add', 1)]}) == 2, 'serialized adds must both land'\nprint('All tests passed!')"
  },
  {
    id: 52, stage: 10, title: "Lock It", pattern: "mutual-exclusion", skill: "critical section = acquire, act, release",
    statement: "The Machine (from P51, given complete) now has a lock: lock_holder is None when free. Add two ops to step: ('acquire',) takes the lock if free — if another thread holds it, return False and stay blocked this pass; ('release',) frees it. Then the test proves it: the same two interleaved increments that lost one update now land both.",
    examples: [
      { input: "A: [acquire, read, add 1, release], B: same", output: "final value 2", explain: "B blocks at acquire until A's critical section ends" },
    ],
    why: "A mutex makes a section of code atomic: only one thread inside at a time, so no one can interleave into your read-modify-write. The acquire-block-release cycle is mutual exclusion, and the simulator's round-robin shows its cost honestly: B stands at the door while A works — blocking is time. The critical section discipline is the interview answer to 'how do you fix a race': identify the shared read-modify-write, wrap exactly it, release promptly.",
    starterCode: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n        self.lock_holder = None\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"add\":\n            self.value = self.registers[tid] + op[1]\n            return True\n        # ── your job: ('acquire',) and ('release',) ──\n        return False",
    hints: [
      "acquire: if lock_holder is not None, return False (blocked). Otherwise lock_holder = tid, return True.",
      "release: lock_holder = None, return True.",
      "Round-robin handles the rest — B's blocked acquire just waits for a free pass.",
    ],
    solution: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n        self.lock_holder = None\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"add\":\n            self.value = self.registers[tid] + op[1]\n            return True\n        if op[0] == \"acquire\":\n            if self.lock_holder is not None:\n                return False\n            self.lock_holder = tid\n            return True\n        if op[0] == \"release\":\n            self.lock_holder = None\n            return True\n        return False",
    walkthrough: "Trace the locked schedule: pass 1 — A acquires (holder=A), B's acquire blocks (holder taken). Every pass after: A advances (read, add, release) while B stands blocked. The moment A releases, B's acquire succeeds — and now B's read sees A's WRITE (value=1), so B's add lands 2. The lock didn't speed anything up; it made the two increments SERIAL, and serial increments can't lose each other. Note the discipline in the schedule shape: acquire, read, add, release — the smallest possible critical section. Holding locks across anything you don't need is how throughput dies and deadlocks are born (P54).",
    testCode: "unlocked = Machine()\nassert unlocked.run({'A': [('read',), ('add', 1)], 'B': [('read',), ('add', 1)]}) == 1, 'without the lock the update is lost'\nlocked = Machine()\nresult = locked.run({\n    'A': [('acquire',), ('read',), ('add', 1), ('release',)],\n    'B': [('acquire',), ('read',), ('add', 1), ('release',)],\n})\nassert result == 2, f'locked critical sections must count both, got {result}'\nprint('All tests passed!')"
  },
  {
    id: 53, stage: 10, title: "The Bounded Buffer", pattern: "producer-consumer", skill: "blocking ops are flow control",
    statement: "The Machine (given, with lock) now carries a bounded buffer: capacity, buffer list, collected list. Implement ('put', x) — append to buffer, or return False (block) when full — and ('get',) — pop the oldest into collected, or block when empty. Then verify two scenarios: two producers and a consumer on capacity 1, and a consumer that starts before any producer.",
    examples: [
      { input: "P1: [put 1, put 2], P2: [put 10], C: [get, get, get], capacity 1", output: "collected == [1, 2, 10]", explain: "P2 waits at the full buffer; FIFO order preserved" },
    ],
    why: "Producer-consumer is the shape of every queue, pipe, and task system. The bounded buffer adds flow control: producers block when the consumer can't keep up (no unbounded memory), consumers block when there's nothing to do (no busy-wait). Blocking IS the coordination — and in the simulator you implement it as literally 'make no progress this pass', which is what a real blocked thread is. FIFO order falls out of append/pop(0) for free.",
    starterCode: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self, capacity=2):\n        self.value = 0\n        self.registers = {}\n        self.lock_holder = None\n        self.capacity = capacity\n        self.buffer = []\n        self.collected = []\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"put\":\n            pass  # block (return False) when full; else append op[1]\n        if op[0] == \"get\":\n            pass  # block (return False) when empty; else pop oldest into collected\n        return False",
    hints: [
      "put: if len(self.buffer) >= self.capacity return False; append and return True.",
      "get: if not self.buffer return False; collected.append(buffer.pop(0)), return True.",
      "The empty-buffer scenario works because round-robin lets the producer run while C is blocked.",
    ],
    solution: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self, capacity=2):\n        self.value = 0\n        self.registers = {}\n        self.lock_holder = None\n        self.capacity = capacity\n        self.buffer = []\n        self.collected = []\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"put\":\n            if len(self.buffer) >= self.capacity:\n                return False\n            self.buffer.append(op[1])\n            return True\n        if op[0] == \"get\":\n            if not self.buffer:\n                return False\n            self.collected.append(self.buffer.pop(0))\n            return True\n        return False",
    walkthrough: "Trace scenario one: P1 puts 1 → buffer [1]. P2 tries to put 10 → full (capacity 1) → blocked, op stays at P2's head. C gets 1. Now P1 puts 2 (buffer full again — P2 still waits), C gets 2, and finally P2's 10 enters and C collects it: [1, 2, 10] — arrival order preserved despite three racing threads. Scenario two inverts it: C's get fires first, finds the buffer empty, blocks; the producer's put in the same pass unblocks it next pass. In production terms you just implemented semaphore semantics for a bounded queue — the core of every worker pool, log pipeline, and message broker. The lock field sits unused here: a deque with append/pop is already atomic in real runtimes — knowing WHICH structures need the lock is half the skill.",
    testCode: "m = Machine(capacity=1)\nm.run({\n    'P1': [('put', 1), ('put', 2)],\n    'P2': [('put', 10)],\n    'C': [('get',), ('get',), ('get',)],\n})\nassert m.collected == [1, 2, 10], m.collected\nassert m.buffer == []\nm2 = Machine(capacity=1)\nm2.run({\n    'C': [('get',)],\n    'P': [('put', 7)],\n})\nassert m2.collected == [7], m2.collected\nprint('All tests passed!')"
  },
  {
    id: 54, stage: 10, title: "The Deadly Embrace", pattern: "deadlock", skill: "circular wait; fix with lock ordering",
    statement: "The Machine (given, complete) now supports NAMED locks: ('acquire', name) / ('release', name) over a locks dict. Your job is not code — it's the schedules. Implement schedules() returning (bad, good): two thread-programs where each worker reads and adds 1 while holding BOTH locks LA and LB. bad takes the locks in opposite orders — round-robin must deadlock (run raises DeadlockError). good takes them in the same order — both workers complete, value ends at 2.",
    examples: [
      { input: "T1 holds LA wants LB; T2 holds LB wants LA", output: "DeadlockError", explain: "each waits for the other: circular wait" },
    ],
    why: "Deadlock needs four conditions (Coffman): mutual exclusion, hold-and-wait, no preemption — and circular wait. The first three are given by the machine; your bad schedule supplies the fourth. The fix is famously mundane: a global lock ORDER. If everyone acquires LA before LB, the cycle can't close. Every 'system froze under load' postmortem with multiple locks traces back here — and the fix is a convention, enforced by review or by a lint rule, not cleverness.",
    starterCode: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n        self.locks = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"add\":\n            self.value = self.registers[tid] + op[1]\n            return True\n        if op[0] == \"acquire\":\n            held = self.locks.get(op[1])\n            if held is not None and held != tid:\n                return False\n            self.locks[op[1]] = tid\n            return True\n        if op[0] == \"release\":\n            self.locks.pop(op[1], None)\n            return True\n        return False\n\n# Two workers must each: acquire LA and LB (in SOME order), read, add 1,\n# release both. Write the dict of thread-programs twice:\n#   bad  — opposite lock orders → round-robin must deadlock\n#   good — same lock order → both complete, final value 2\n\ndef schedules():\n    bad = {}\n    good = {}\n    return bad, good",
    hints: [
      "bad: T1 acquires LA then LB; T2 acquires LB then LA. Round-robin: T1 gets LA, T2 gets LB, then both block forever.",
      "good: BOTH take LA first, then LB. T2 simply waits while T1 works.",
      "Each worker's program: [('acquire', X), ('acquire', Y), ('read',), ('add', 1), ('release', X), ('release', Y)].",
    ],
    solution: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self):\n        self.value = 0\n        self.registers = {}\n        self.locks = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"add\":\n            self.value = self.registers[tid] + op[1]\n            return True\n        if op[0] == \"acquire\":\n            held = self.locks.get(op[1])\n            if held is not None and held != tid:\n                return False\n            self.locks[op[1]] = tid\n            return True\n        if op[0] == \"release\":\n            self.locks.pop(op[1], None)\n            return True\n        return False\n\ndef schedules():\n    bad = {\n        \"T1\": [(\"acquire\", \"LA\"), (\"acquire\", \"LB\"), (\"read\",), (\"add\", 1), (\"release\", \"LA\"), (\"release\", \"LB\")],\n        \"T2\": [(\"acquire\", \"LB\"), (\"acquire\", \"LA\"), (\"read\",), (\"add\", 1), (\"release\", \"LB\"), (\"release\", \"LA\")],\n    }\n    good = {\n        \"T1\": [(\"acquire\", \"LA\"), (\"acquire\", \"LB\"), (\"read\",), (\"add\", 1), (\"release\", \"LA\"), (\"release\", \"LB\")],\n        \"T2\": [(\"acquire\", \"LA\"), (\"acquire\", \"LB\"), (\"read\",), (\"add\", 1), (\"release\", \"LA\"), (\"release\", \"LB\")],\n    }\n    return bad, good",
    walkthrough: "Trace bad one pass at a time: T1 acquires LA. T2 acquires LB. T1 asks for LB — held by T2, blocked. T2 asks for LA — held by T1, blocked. No thread can advance; run() raises DeadlockError — the simulator's answer to 'both threads frozen forever'. The wait-for graph is a cycle: T1 → LB → T2 → LA → T1. Now good: both take LA first. T2's first acquire blocks while T1 does everything — no cycle can form because there's always a lock everyone agrees to want FIRST. Lock ordering is the whole fix; databases apply it as 'touch tables in a consistent order', the Linux kernel as lock hierarchies. In interviews, naming the four Coffman conditions AND the ordering fix is the complete answer.",
    testCode: "bad, good = schedules()\nm1 = Machine()\ntry:\n    m1.run(bad)\n    assert False, \"opposite lock orders must deadlock\"\nexcept DeadlockError:\n    pass\nm2 = Machine()\nassert m2.run(good) == 2, f'ordered locks must complete, got {m2.value}'\nprint('All tests passed!')"
  },
  {
    id: 55, stage: 10, title: "The Watched Meter", pattern: "thread-safe-design", skill: "check-and-act inside the lock",
    statement: "Final composition: the Machine (given) tracks a meter — initial credits in value, total spent in spent. Implement the ('spend', n) op: if the thread's register (from an earlier read) holds >= n credits, subtract n from value and add n to spent; either way the op executes. Then write two schedules spending 2 credits per thread from an initial 3: race_schedules (bare read-then-spend — round-robin must overspend, spent == 4) and safe_schedules (each read-spend wrapped in acquire/release of lock 'M' — the second spend is correctly rejected, spent == 2).",
    examples: [
      { input: "A: [read, spend 2], B: [read, spend 2], unlocked, start 3", output: "spent == 4 — both read stale 3", explain: "locked: A spends 2, B reads 1 and is refused → spent == 2" },
    ],
    why: "The rate-limiter question, asked in every backend interview, in its honest form: the danger isn't the decrement — it's the CHECK between the read and the act. Unlocked, both threads read 3 credits, both pass the check, both spend: 4 credits gone from a 3-credit meter. The fix is placement: check-and-act must sit INSIDE the same critical section as the read. This is the LLD ladder's closing move — you built the race, built the lock, and now know exactly why the lock must wrap the check.",
    starterCode: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self, initial=0):\n        self.value = initial\n        self.spent = 0\n        self.registers = {}\n        self.locks = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"acquire\":\n            held = self.locks.get(op[1])\n            if held is not None and held != tid:\n                return False\n            self.locks[op[1]] = tid\n            return True\n        if op[0] == \"release\":\n            self.locks.pop(op[1], None)\n            return True\n        if op[0] == \"spend\":\n            pass  # ── your job: check the register, conditionally spend ──\n        return False\n\n# The meter starts at 3. Each thread spends 2.\n#   race_schedules: bare [read, spend 2] per thread (no lock)\n#   safe_schedules: [acquire M, read, spend 2, release M] per thread\n\ndef race_schedules():\n    return {}\n\ndef safe_schedules():\n    return {}",
    hints: [
      "spend: credits = self.registers.get(tid, 0); if credits >= op[1]: value = credits - op[1]; spent += op[1]. Return True either way.",
      "race_schedules: {'A': [('read',), ('spend', 2)], 'B': [('read',), ('spend', 2)]}.",
      "safe_schedules: same ops with ('acquire', 'M') first and ('release', 'M') last.",
    ],
    solution: "class DeadlockError(Exception):\n    pass\n\nclass Machine:\n    def __init__(self, initial=0):\n        self.value = initial\n        self.spent = 0\n        self.registers = {}\n        self.locks = {}\n\n    def run(self, threads):\n        pending = {tid: list(ops) for tid, ops in threads.items()}\n        while any(pending.values()):\n            progressed = False\n            for tid, ops in pending.items():\n                if ops and self.step(tid, ops[0]):\n                    ops.pop(0)\n                    progressed = True\n            if not progressed:\n                raise DeadlockError(\"no thread can advance\")\n        return self.value\n\n    def step(self, tid, op):\n        if op[0] == \"read\":\n            self.registers[tid] = self.value\n            return True\n        if op[0] == \"acquire\":\n            held = self.locks.get(op[1])\n            if held is not None and held != tid:\n                return False\n            self.locks[op[1]] = tid\n            return True\n        if op[0] == \"release\":\n            self.locks.pop(op[1], None)\n            return True\n        if op[0] == \"spend\":\n            credits = self.registers.get(tid, 0)\n            if credits >= op[1]:\n                self.value = credits - op[1]\n                self.spent += op[1]\n            return True\n        return False\n\ndef race_schedules():\n    return {\n        \"A\": [(\"read\",), (\"spend\", 2)],\n        \"B\": [(\"read\",), (\"spend\", 2)],\n    }\n\ndef safe_schedules():\n    return {\n        \"A\": [(\"acquire\", \"M\"), (\"read\",), (\"spend\", 2), (\"release\", \"M\")],\n        \"B\": [(\"acquire\", \"M\"), (\"read\",), (\"spend\", 2), (\"release\", \"M\")],\n    }",
    walkthrough: "Race trace: pass 1 — A reads 3 into its register, B reads 3 into ITS register. Pass 2 — A spends 2 (3 >= 2, value 1, spent 2), B spends 2 (B's register still says 3! value 1, spent 4). A 3-credit meter paid out 4 — the check passed on stale data, the exact DB-ladder P48 'one writer at a time' story in userland. Safe trace: A holds M through read-check-spend; B blocks at acquire, then reads the POST-spend value 1, fails the check honestly, spends nothing: spent == 2, value == 1, invariant intact. The lesson generalizes beyond meters: token buckets, connection pools, inventory reservation, bank checks — anywhere code reads a value, decides, then writes, the decision must live inside the lock that guards the value. That placement discipline is the entire craft of thread-safe design.",
    testCode: "bad = Machine(initial=3)\nbad.run(race_schedules())\nassert bad.spent == 4, f'unlocked must overspend, spent {bad.spent}'\ngood = Machine(initial=3)\ngood.run(safe_schedules())\nassert good.spent == 2, f'locked must refuse the second spend, spent {good.spent}'\nassert good.value == 1, good.value\nprint('All tests passed!')"
  },
]
