  ...
  stages: {
    ...
    design: {
      contract: {
        ...
        complexity: {
          prompt: "For n = 6, count the calls in your head: 6 → 5 → 4 → 3 → 2 → 1. How many, and what does that make the total cost?",
          options: [
            { label: "6 calls, O(1) work each → O(n)", value: "linear" },
            { label: "1 call → O(1)", value: "constant" },
            { label: "6×6 = 36 calls → O(n²)", value: "quadratic" },
          ],
          correct: "linear",
          wrongFeedback: "Count the chain links, not the numbers: each call spawns exactly one smaller call until the floor. How many links for n?",
